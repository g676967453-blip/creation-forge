  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const abs = pathResolve(p);
        const fsx = await import("node:fs/promises");
        await fsx.rm(abs, { recursive: true, force: false });
        json(res, 200, { ok: true, path: abs });
      } catch (err) {
        json(res, 500, { error: String(err) });
      }
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (!from || !to) { json(res, 400, { error: "missing from/to" }); return; }
        const absFrom = pathResolve(from);
        const absTo = pathResolve(to);
        const fsx = await import("node:fs/promises");
        await fsx.rename(absFrom, absTo);
        json(res, 200, { ok: true, from: absFrom, to: absTo });
      } catch (err) {
        json(res, 500, { error: String(err) });
      }
    }
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const absTo = pathResolve(to);
        const fsx = await import("node:fs/promises");
        await fsx.cp(absFrom, absTo, { recursive: true, errorOnExist: true, force: false });
        json(res, 200, { ok: true, from: absFrom, to: absTo });
      } catch (err) {
        json(res, 500, { error: String(err) });
      }
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          if (process.platform === "win32") {
            execFile("cmd.exe", ["/c", "start", "", abs], { windowsHide: true }, (err) => err ? reject(err) : resolve(null));
          } else if (process.platform === "darwin") {
            execFile("open", [abs], (err) => err ? reject(err) : resolve(null));
          } else {
            execFile("xdg-open", [abs], (err) => err ? reject(err) : resolve(null));
          }
        });
        json(res, 200, { ok: true, path: abs });
      } catch (err) {
