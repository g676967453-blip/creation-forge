...
| 我要… | 去哪 | 例子 |
|------|------|------|
| 看今天待办 / 改目标 | **本目录根** | [个人待办.md](./个人待办.md) · [目标规划.md](./目标规划.md) |
...
| 看数据仪表盘 | 根目录 [index.html](./index.html) | ⚠️ 这是**仪表盘**，不是文档树 |

仓根还有日常产出：**`works/`**（一事一记，不属于 docs）。
...
docs/
├── README.md                 ← 你在这里
├── 个人待办.md / 目标规划.md   ← 高频活状态
├── index.html                ← 造化坊仪表盘（≠ 知识库首页）
├── zh-CN/                    ← 宪法 + 工程规范
...
| **IAA / 买量** | [knowledge · IAA](./knowledge/README.md#iaa--买量与合规) | — | 相关待办在个人待办 |
| **目标 / 待办** | 数据：根目录两表 | — | [目标管理](./workflows/目标管理.md) · [个人待办管理](./workflows/个人待办管理.md) |
...
4. **禁止**用 `docs/index.html` 当文档目录；改导航优先改本 README 与各子 README。  
5. 改 `个人待办` / `目标规划` / 根协作协议前检查 `.ai-locks/`。
