...
- 平台目录：`memory/`（跨 AI 记忆）、`.ai-locks/`（文件锁）、`.claude/`（Claude Code 配置/SKILL）、`.vscode/`（编辑器配置）、`templates/`（项目模板）、`shared/`（共享库）、`tools/`（平台工具：scaffold-game.ts / check / convert-encoding / extract_pdf.py / batch-removebg.py / _archive）、`pdf_libs/`、`node_modules/`（`.github/` 已空——仪表盘 GH Pages 部署 2026-09-06 下线，见 [造化仪表盘/CLAUDE.md](./造化仪表盘/CLAUDE.md)）
