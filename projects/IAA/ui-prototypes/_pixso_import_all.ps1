...
function Connect-Mcp {
  $init = @{
    ...
    params = @{
      ...
      capabilities = @{}
      clientInfo = @{ name = 'dsh-import'; version = '1' }
    }
...
$clear = Invoke-McpTool -Name 'eval_script' -Arguments @{ script = @'
const page = pixso.currentPage;
page.name = "FireHero-UI-Prototypes";
const ids = page.children.map(n => n.id);
for (const id of ids) {
  const n = pixso.getNodeById(id);
  if (n) n.remove();
}
return { cleared: ids.length, pageId: page.id, pageName: page.name };
'@ }
Write-Output ('CLEAR: ' + (Get-McpText $clear))

$screens = @(
  @{ file = '01-main-menu.html'; name = '01-MainMenu'; col = 0; row = 0 }
  @{ file = '02-in-game-hud.html'; name = '02-InGameHUD'; col = 1; row = 0 }
  @{ file = '03-character-select.html'; name = '03-CharacterSelect'; col = 2; row = 0 }
  @{ file = '04-level-clear.html'; name = '04-LevelClear'; col = 3; row = 0 }
...
$idsRaw = Invoke-McpTool -Name 'eval_script' -Arguments @{ script = 'return pixso.currentPage.children.slice(0,3).map(n=>n.id);' }
$idsText = Get-McpText $idsRaw
Write-Output ('SHOT_IDS: ' + $idsText)
try {
  $ids = @($idsText | ConvertFrom-Json)
  if ($ids.Count -gt 0) {
    $shot = Invoke-McpTool -Name 'take_screenshot' -Arguments @{ nodeIds = $ids } -TimeoutSec 120
    $shotPath = 'J:\ceshi\projects\IAA\ui-prototypes\pixso-previews\first3.json'
    New-Item -ItemType Directory -Force -Path (Split-Path $shotPath) | Out-Null
    $shotText = Get-McpText $shot
    [IO.File]::WriteAllText($shotPath, $shotText, [Text.UTF8Encoding]::new($false))
    Write-Output ("SHOT_SAVED bytes=" + ([IO.File]::ReadAllBytes($shotPath).Length))
  }
} catch {
  Write-Output ('SHOT_FAIL: ' + $_.Exception.Message)
}
