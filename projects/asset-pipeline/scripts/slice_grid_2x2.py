"""
2×2 网格切片脚本
用途：将 Lovart 生成的 2048×2048 2×2 网格图切割为 4 个独立 1024×1024 PNG

用法：
  python slice_grid_2x2.py --input grid.png --output-dir ./buildings --prefix building

输出：
  building_r0_c0.png, building_r0_c1.png, building_r1_c0.png, building_r1_c1.png
"""
...
def slice_2x2(
    ...
    """
    将 2048×2048 图片等分为 4 个 1024×1024 切片。
    ...
    if width != 2048 or height != 2048:
        print(f"[slice] WARNING: input {width}x{height}, not standard 2048x2048, dividing by actual size")
        cell_w = width // COLS
        cell_h = height // ROWS
    else:
    ...
    print(f"[slice] Grid: {ROWS}x{COLS}, cell size {cell_w}x{cell_h}")
...
def main():
    parser = argparse.ArgumentParser(
        description="2×2 网格切片：2048×2048 → 4×1024×1024"
    )
    parser.add_argument("--input", "-i", required=True, help="输入图片路径（2048×2048 网格图）")
    parser.add_argument("--output-dir", "-o", required=True, help="输出目录")
    parser.add_argument("--prefix", "-p", default="building", help="输出文件名前缀")
