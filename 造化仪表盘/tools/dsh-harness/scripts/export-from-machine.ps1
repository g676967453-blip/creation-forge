...
# Export current machine DSH portable config into this pack (no worktable plugin).
param(
...
if (-not (Test-Path (Join-Path $PackRoot 'profiles'))) {
...
foreach ($p in @('desktop', 'web')) {
  ...
  # strip dsh-worktable if present; keep base bundles only
  ...
  if (Test-Path $pkgPath) {
    ...
    if ($pkg.dependencies.PSObject.Properties.Name -contains 'dsh-worktable') {
      $pkg.dependencies.PSObject.Properties.Remove('dsh-worktable')
    }
    if (-not $pkg.dsh) {
      $pkg | Add-Member dsh ([pscustomobject]@{ profile = ([pscustomobject]@{ bundles = @() }) }) -Force
    }
    if (-not $pkg.dsh.profile) {
      $pkg.dsh | Add-Member profile ([pscustomobject]@{ bundles = @() }) -Force
    }
    $bundles = @($pkg.dsh.profile.bundles) | Where-Object { $_ -and $_ -ne 'dsh-worktable' }
    foreach ($b in @('@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app')) {
      if ($bundles -notcontains $b) { $bundles = @($b) + @($bundles) }
    }
    $pkg.dsh.profile.bundles = @($bundles | Select-Object -Unique)
    $json = $pkg | ConvertTo-Json -Depth 10
    [IO.File]::WriteAllText($pkgPath, $json, (New-Object System.Text.UTF8Encoding $false))
    Write-Info "normalized $p package.json (worktable removed)"
  }
}

# 2) settings example (no secrets)
...
Write-Info 'DONE. Next: commit 造化仪表盘/tools/dsh-harness, then on other machine run install-to-machine.ps1'
