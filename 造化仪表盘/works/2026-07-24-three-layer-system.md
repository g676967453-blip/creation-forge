...
- **SKILL 递归统计**：修正 `countFiles` 为递归 `countSkills`，从 6 → 53（标准 49 + 扁平 4）
- **SKILL vs 工作流区分**：仪表盘工作流清单 SKILL 列改为色标（绿色 badge = 有覆盖，灰色 = 无）
- **变更日志**：新建 `docs/workflows/变更日志.md`，23 条记录覆盖 07-15 → 07-24
- **仪表盘**：系统总览 SKILL 卡片增加变更日志链接
